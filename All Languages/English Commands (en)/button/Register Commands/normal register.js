...
module.exports = {
    ...
    async execute({
                ...
                .then(async messages => {
                    ...
                    await member.edit({
                                ...
                                .addFields(
                                    ...
                                    },
                                    {
                                        ...
                                        value: `> 👤 **Name:** <@${memberId}>\n` +
                                            ...
                                            `> ${EMOJIS.role} **Given role(s):** ${normalRolesString}`,
                                        inline: true
                                    }
                                )
                                .setThumbnail(memberAvatar)
                                .setFooter({
                                    text: `${int.client.user.username} Register system`,
                            ...
                            if (afterRegisterChannelId) {
                                ...
                                if (channel) {
                                    ...
                                    // Eğer özel mesaj ayarlanmamışsa mesajı gönder
                                        ...
                                        const embed = new EmbedBuilder()
                                            .setTitle(`Welcome aboard ${recreateMemberName} ${EMOJIS.hi}`)
                                            .setDescription(`${EMOJIS.crazy} **• <@${memberId}> joined us with the roles ${normalRolesString}**`)
                                            .addFields(
                                                {
                                                    name: "Registration information",
                                                    value: `**• Registered person:** ${recreateMemberName}\n` +
                                ...
                                if (logChannel) {
                                    ...
                                    const embed = new EmbedBuilder()
                                        ...
                                        .setDescription(
                                            ...
                                            `**• Register count:** ${toHumanizeRegisterCount} - (${EMOJIS.normal} ${authorData.countables.normal || 0})\n` +
                                            ...
                                            `**• Given role(s):** ${normalRolesString}\n` +
                                            `**• New name:** ${recreateName}\n` +
                                            `**• Registration type:** Member ${EMOJIS.normal}\n` +
                                            `**• Has the member been registered before:** ${memberPrevNames.length > 0 ? `Yes ${memberPrevNames.length} times` : "No"}`
                                        )
                                        .setThumbnail(memberAvatar)
                                        .setColor(`#${colors.normal}`)
                                        .setFooter({
                                            text: `${int.client.user.username} Log system`,
                                            iconURL: clientAvatar
                                        })
                                        .setTimestamp()
                        ...
                        .catch(async err => {
                            ...
                            if (err.code == RESTJSONErrorCodes.UnknownMember) return waitMessage.reply("• Wellyyy... I think this person is no longer on the server, you stupid thing :(");
        ...
        if (guildDatabase.register.type != "normal") return errorEmbed(
            `My record type is set to __**Gender**__! Please use \`${prefix}boy\` or \`${prefix}girl\` command` +
            (intMember.permissions.has("Administrator") ?
        ...
        if (normalRoleIds.length == 0) return errorEmbed(
            `Member roles are __ not setted__ on this server` +
            (intMember.permissions.has("Administrator") ?
                `\n\n• To set it you can type **${prefix}member-role @role** or instead of setting everything one by one, you can type **${prefix}set** and set up the entire registry system with a single command` :
                "")
        );
        ...
        int.client.buttonRegisterMember.set(`${guildId}.${memberId}`, authorId);
        int.message.reply(
            `${EMOJIS.normal} <@${authorId}>, Please write **only the name${guildDatabase.register.isAgeRequired ? " and age" : ""}** of the person <@${memberId}> you want to register as a message`,
        )
