...
module.exports = {
    ...
    description: "Shows the user's profile photo", // Komutun açıklaması
    category: "Extra commands", // Komutun kategorisi (yardım menüsü için)
    usage: "<px>pp <@user or User ID", // Komutun kullanım şekli
    ...
    async execute({
        ...
        function createEmbed(imageURL, isMemberOrUser) {
            ...
            const allButtons = new ActionRowBuilder()
                .addComponents(
                    new ButtonBuilder()
                        .setLabel(`Open in browser${isMemberHasAvatar ? ` (${isMemberOrUser == "member" ? "Guild" : "Discord"})` : ""}`)
