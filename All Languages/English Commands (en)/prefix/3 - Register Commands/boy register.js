...
module.exports = {
    ...
    description: "Registers the person as a boy", // Komutun açıklaması
    category: "Register commands", // Komutun kategorisi (yardım menüsü için)
    usage: "<px>boy <@user or User ID> <New name>", // Komutun kullanım şekli
    ...
    async execute({
        ...
        if (member.user.bot) {
            if (guildDatabase.register.roleIds.bot) return errorEmbed(
                `You can't register a bot as boy, you stupid thing\n\n` +
        ...
        if (!memberName) return errorEmbed(
            ...
            `**Example**\n` +
            `• ${prefix}boy ${memberId} Fearless Crazy 20\n` +
            `• ${prefix}boy <@${memberId}> Fearless Crazy 20\n` +
            `• ${prefix}boy Fearless Crazy 20 <@${memberId}>`
        );
