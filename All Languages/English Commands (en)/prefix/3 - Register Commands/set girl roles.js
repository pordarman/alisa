...
module.exports = {
    ...
    description: "You set the girl roles", // Komutun açıklaması
    category: "Register commands", // Komutun kategorisi (yardım menüsü için)
    usage: "<px>girlrole <@roles or Role IDs>", // Komutun kullanım şekli
    ...
    async execute({
        ...
        if (content.toLocaleLowerCase(language) == "reset") {
            ...
            if (roleIds.girl.length == 0) return errorEmbed("The roles given to girls have already been reset");

            roleIds.girl = [];
            database.writeFile(guildDatabase, guildId);
            return errorEmbed("Roles assigned to girls have been successfully reset", "success");
        }

        // Rolü ayarla
        const allRoles = Util.fetchRoles(msg);
        if (allRoles.size == 0) return errorEmbed(
            `• To set girl roles **${prefix}${this.name} @role @role @role**\n\n` +
        ...
        database.writeFile(guildDatabase, guildId);
        return errorEmbed(
            `From now on I will give girls role(s) named [${allRoles.map(role => `<@&${role.id}>`).join(" | ")}]`,
            "success"
        );
