...
module.exports = {
    name: "member", // Komutun ismi
    id: "kayıt", // Komutun ID'si
    cooldown: 2, // Komutun bekleme süresi
    aliases: [ // Komutun diğer çağırma isimleri
        "member"
    ],
    description: "Registers the person as a member", // Komutun açıklaması
    category: "Register commands", // Komutun kategorisi (yardım menüsü için)
    usage: "<px>member <@user or User ID> <New name>", // Komutun kullanım şekli
    ...
    async execute({
        ...
        if (member.user.bot) {
            if (guildDatabase.register.roleIds.bot) return errorEmbed(
                `You can't register a bot as member, you stupid thing\n\n` +
        ...
        if (!memberName) return errorEmbed(
            ...
            `**Example**\n` +
            `• ${prefix}member ${memberId} Fearless Crazy 20\n` +
            `• ${prefix}member <@${memberId}> Fearless Crazy 20\n` +
            `• ${prefix}member Fearless Crazy 20 <@${memberId}>`
        );
