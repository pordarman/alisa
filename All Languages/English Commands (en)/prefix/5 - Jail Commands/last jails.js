...
module.exports = {
    ...
    description: "Shows the server's last jail time", // Komutun açıklaması
    category: "Jail commands", // Komutun kategorisi (yardım menüsü için)
    usage: "<px>lastjail [@user or User ID]", // Komutun kullanım şekli
    ...
    async execute({
        ...
        if (!length) return errorEmbed(
            user ?
                "The table cannot be displayed because the person you tagged has never been jailed before" :
                "The table cannot be displayed because no one has been jailed on this server before"
        );
                ...
                try {
                    ...
                    resultArray.push(
                        ...
                        `• \`#${length - index}\` ${isTempJailed ? "⏰ " : ""}${isJailed ? "📥" : "📤"} <@${authorId}> ${user ? `<@${memberId}> ` : ""}| <t:${timestamp}:F>${isJailed ?
                            `\n└> **Reason:** ${truncatedString(reason || "Reason not stated", 50)}`
                            : ""
                        }${isTempJailed ?
        ...
        function createEmbed(pageNum) {
            ...
            return new EmbedBuilder()
                ...
                .setDescription(
                    `**• ${embedDescription} __${LENGTH_TO_HUMANIZE}__ jail information found\n\n` +
