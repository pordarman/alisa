...
module.exports = {
    ...
    description: "Kayıt kanalını ayarlarsınız", // Komutun açıklaması
    category: "Kayıt komutları", // Komutun kategorisi (yardım menüsü için)
    usage: "<px>kayıtkanal <#kanal veya Kanal ID'si>", // Komutun kullanım şekli
    ...
    async execute({
        ...
        if (["sıfırla", "sifirla"].includes(content.toLocaleLowerCase(language))) {
            ...
            if (!channelIds.register) return errorEmbed("Kayıt kanalı zaten sıfırlanmış durumda");

            channelIds.register = "";
            database.writeFile(guildDatabase, guildId);
            return errorEmbed("Kayıt kanalı başarıyla sıfırlandı", "success");
        }

        // Rolü ayarla
        const channel = Util.fetchChannel(msg);
        if (!channel) return errorEmbed(
            `• Kayıt kanalını ayarlamak için **${prefix}${this.name} #kanal**\n\n` +
        ...
        if (channel.id == channelIds.register) return errorEmbed("Kayıt kanalı zaten etiketlediğiniz kanalla aynı");

        channelIds.register = channel.id;
        database.writeFile(guildDatabase, guildId);
        return errorEmbed(
            `Bundan sonra kayıtlar sadece <#${channel.id}> kanalında gerçekleştirilecek`,
            "success"
        );
