...
module.exports = {
    ...
    description: "Kişiyi erkek olarak kayıt eder", // Komutun açıklaması
    category: "Kayıt komutları", // Komutun kategorisi (yardım menüsü için)
    usage: "<px>erkek <@kişi veya Kişi ID'si> <Yeni ismi>", // Komutun kullanım şekli
    ...
    async execute({
        ...
        if (member.user.bot) {
            if (guildDatabase.register.roleIds.bot) return errorEmbed(
                `Bir botu erkek olarak kayıt edemezsin şapşik şey seni\n\n` +
        ...
        if (!memberName) return errorEmbed(
            ...
            `**Örnek**\n` +
            `• ${prefix}e ${memberId} Fearless Crazy 20\n` +
            `• ${prefix}e <@${memberId}> Fearless Crazy 20\n` +
            `• ${prefix}e Fearless Crazy 20 <@${memberId}>`
        );
                ...
                if (afterRegisterChannelId) {
                    ...
                    if (channel) {
                        ...
                        // Eğer özel mesaj ayarlanmamışsa mesajı gönder
                            ...
                            const embed = new EmbedBuilder()
                                .setTitle(`Aramıza hoşgeldin ${recreateMemberName} ${EMOJIS.hi}`)
                                .setDescription(`${EMOJIS.crazy} **• <@${memberId}> aramıza ${boyRolesString} rolleriyle katıldı**`)
                                .addFields(
                                    {
                                        name: "Kaydın bilgileri",
                                        value: `**• Kayıt edilen kişi:** ${recreateMemberName}\n` +
                    ...
                    if (logChannel) {
                        ...
                        const embed = new EmbedBuilder()
                            ...
                            .setDescription(
                                ...
                                `**• Verilen rol(ler):** ${boyRolesString}\n` +
                                `**• Yeni ismi:** ${recreateName}\n` +
                                `**• Kayıt şekli:** Erkek ${EMOJIS.boy}\n` +
