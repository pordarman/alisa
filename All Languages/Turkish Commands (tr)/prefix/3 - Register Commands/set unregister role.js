...
module.exports = {
    ...
    description: "Kayıtsız rolünü ayarlarsınız", // Komutun açıklaması
    category: "Kayıt komutları", // Komutun kategorisi (yardım menüsü için)
    usage: "<px>kayıtsızrol <@rol veya Rol ID'si>", // Komutun kullanım şekli
    ...
    async execute({
        ...
        if (["sıfırla", "sifirla"].includes(content.toLocaleLowerCase(language))) {
            ...
            if (!roleIds.unregister) return errorEmbed("Kayıtsız üye rolü zaten sıfırlanmış durumda");

            roleIds.unregister = "";
            database.writeFile(guildDatabase, guildId);
            return errorEmbed("Kayıtsız üye rolü başarıyla sıfırlandı", "success");
        }

        // Rolü ayarla
        const role = Util.fetchRole(msg);
        if (!role) return errorEmbed(
            `• Kayıtsız üye rolünü ayarlamak için **${prefix}${this.name} @rol**\n\n` +
        ...
        if (roleIds.unregister === rolId) return errorEmbed("Kayıtsız üye rolü zaten etiketlediğiniz rolle aynı");
        ...
        if (roleIds.yetkili == rolId) return errorEmbed(`Etiketlediğiniz rol bu sunucudaki üyeleri kayıt eden rol. Lütfen başka bir rol etiketleyiniz`);

        // Rolün sırası botun sırasından yüksekse hata döndür
        const highestRole = guildMe.roles.highest;
        if (highestRole.position <= role.position) return errorEmbed(`<@&${rolId}> adlı rolün sırası benim rolümün sırasından yüksek! Lütfen <@&${Util.getBotOrHighestRole(guildMe).id}> adlı rolü üste çekiniz ve tekrar deneyiniz`)

        roleIds.unregister = rolId;
        database.writeFile(guildDatabase, guildId);
        return errorEmbed(
            `Bundan sonra üyeleri kayıt ettikten sonra onlardan <@&${rolId}> adlı rolü alacağım\n\n` +
            `• ‼️ **Uyarı!** Bu rol sadece __**kayıt kanalı**__ ayarlı olduğu zaman verilecektir${guildDatabase.register.channelIds.register ? "" : `• Kayıt kanalını ayarlamak için **${prefix}kayıtkanal #kanal** yazabilirsiniz`}`,
            "success"
        );
