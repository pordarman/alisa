...
module.exports = {
    ...
    description: "Jail log kanalını ayarlarsınız", // Komutun açıklaması
    category: "Jail komutları", // Komutun kategorisi (yardım menüsü için)
    usage: "<px>jaillog <#kanal veya Kanal ID'si>", // Komutun kullanım şekli
    ...
    async execute({
        ...
        if (["sıfırla", "sifirla"].includes(content.toLocaleLowerCase(language))) {
            ...
            if (!jail.logChannelId) return errorEmbed("Jail log kanalı zaten sıfırlanmış durumda");

            jail.logChannelId = "";
            database.writeFile(guildDatabase, guildId);
            return errorEmbed("Jail log kanalı başarıyla sıfırlandı", "success");
        }

        // Rolü ayarla
        const channel = Util.fetchChannel(msg);
        if (!channel) return errorEmbed(
            `• Jail log kanalını ayarlamak için **${prefix}${this.name} #kanal**\n\n` +
        ...
        if (channel.id == jail.logChannelId) return errorEmbed("Jail log kanalı zaten etiketlediğiniz kanalla aynı");

        jail.logChannelId = channel.id;
        database.writeFile(guildDatabase, guildId);
        return errorEmbed(
            `Bundan sonra bütün Jail loglarını (Eğer komut Alisa üzerinden kullanılmışsa) <#${channel.id}> kanalına atacağım`,
            "success"
        );
