...
module.exports = {
    ...
    description: { // Komutun açıklaması
        tr: "Yaş zorunluluğunu açıp kapatırsınız",
        en: "You can turn the age requirement on or off"
    },
    ...
    usage: { // Komutun kullanım şekli
        tr: "<px>yaşzorunlu <aç veya kapat>",
    ...
    async execute({
        ...
        switch (onOffSwitchs(args[0]?.toLocaleLowerCase(language))) {
            ...
            case "on":
                ...
                if (guildDatabase.register.isAgeRequired) return errorEmbed(onOffMessages.alreadyOn);

                // Database'ye kaydet
                guildDatabase.register.isAgeRequired = true;
                await database.updateGuild(guildId, {
                    $set: {
                        "register.isAgeRequired": true
                    }
                });
            ...
            case "off":
                ...
                if (!guildDatabase.register.isAgeRequired) return errorEmbed(onOffMessages.alreadyOff);

                // Database'ye kaydet
                guildDatabase.register.isAgeRequired = false;
                await database.updateGuild(guildId, {
                    $set: {
                        "register.isAgeRequired": false
                    }
                });
