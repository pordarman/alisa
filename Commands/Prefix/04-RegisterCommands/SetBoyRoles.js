...
module.exports = {
    ...
    description: { // Komutun açıklaması
        tr: "Erkek rollerini ayarlarsınız",
        en: "You set boy roles"
    },
    ...
    usage: { // Komutun kullanım şekli
        tr: "<px>erkekrol <@roller veya Rollerin ID'leri>",
        en: "<px>boyrole <@roles or Role IDs>"
    },
    ...
    async execute({
        ...
        if (resetSet.has(content.toLocaleLowerCase(language))) {
            ...
            if (roleIds.boy.length == 0) return errorEmbed(roleMessages.alreadyReset(roleNames.boy));
            ...
            return errorEmbed(roleMessages.successReset(roleNames.boy), "success");
        }
        ...
        await database.updateGuild(guildId, {
            $set: {
                "register.roleIds.boy": roleIds.boy
            }
        });
        return errorEmbed(
