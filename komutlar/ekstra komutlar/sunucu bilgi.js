  ...
  async run({ guildDatabase, pre, alisa, msg, args, guildId, prefix, hata, guild, msgMember, guildMe }) {
        try {
            let discordlogo = guild.iconURL()
                ...
                , üyeler = await msg.client.getMembers(msg)
                , emojiler = guild.emojis.cache
                , everyoneHariçDigerRoller = guild.roles.cache.filter(a => a.id !== guild.id)
                , a = everyoneHariçDigerRoller.sort((a, b) => b.position - a.position).map(a => `<@&${a.id}>`).slice(0, 20).join(" | ") + (everyoneHariçDigerRoller.size > 20 ? `+${everyoneHariçDigerRoller.size - 20} rol daha...` : "")
                , temelBilgi = [
                    `🆔 **Sunucunun ID'si:**  ${guild.id}`,
                    `📅 **Sunucunun kuruluş tarihi:**  <t:${(guild.createdTimestamp / 1000).toFixed(0)}:F> - <t:${(guild.createdTimestamp / 1000).toFixed(0)}:R>`,
                    `🔔 **Sunucunun varsayılan mesaj bildirimleri:**  ${guild.defaultMessageNotifications == "ALL_MESSAGES" ? "Bütün mesajlar 📬" : "Sadece etiketler ${ayarlar.emoji.rol}"}`
                ]
