    ...
    async run({ sunucudb, pre, alisa, msg, args, sunucuid, prefix, hata, guild, msgMember, guildMe }) {
        try {
            ...
            if (["rolver", "ver", "ekle", "rolekle"].includes(args[0])) {
                ...
                if (rol.position >= guildMe.roles.highest.position) return hata(`${rol.id == sunucudb.kayıt.kayıtsız ? "Bu sunucudaki kayıtsız rolü " : "Etiketlediğiniz rol "}benim rolümün sırasından yüksek! Lütfen ${guildMe.roles.botRole?.toString() || guildMe.roles.highest?.toString()} adlı rolü üste çekiniz ve tekrar deneyiniz`)
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`• Bu sunucuda rolleri olmayan toplam **${members.size}** kişi bulunuyor\n\n• Rolsüz kişilere rol vermek için **${prefix}rolsüz ver @rol** ${sunucudb.kayıt.kayıtsız ? `ya da kayıtsız rolü vermek için **${prefix}rolsüz ver** ` : ""}yazabilirsiniz`)
                .setColor("Blue")
                .setTimestamp()
