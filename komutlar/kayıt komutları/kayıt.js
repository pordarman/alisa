...
module.exports = {
  ...
  async run({ guildDatabase, pre, alisa, msg, args, guildId, prefix, hata, guild, msgMember, guildMe }) {
    try {
      ...
      if (!guildDatabase.kayıt.secenek) return hata(`Kayıt seçeneğim __**Cinsiyet**__ olarak ayarlı lütfen \`${prefix}e\` ve \`${prefix}k\` komutlarını kullanınız${msgMember.permissions.has('Administrator') ? `\n\n• Eğer üye olarak kayıt etmek isterseniz **${prefix}seç normal** yazabilirsiniz` : ""}`)
      ...
      const buttonCooldown = msg.client.buttonCooldown.get(memberid + guildId)
      if (buttonCooldown) {
