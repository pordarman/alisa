  ...
  async run({ sunucudb, pre, alisa, msg, args, sunucuid, prefix, hata, sonradan, guild, msgMember, guildMe }) {
    try {
      ...
      if (sonradan) {
        ...
        yazılacaksunucudb = sonradan.idler
        switch (fun) {
          case 150:
      ...
      async function son(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 11, date: Date.now() }, "kur", "diğerleri")
          ...
          .addFields(
            ...
            },
            {
              ...
              value: [
                ...
                `**• Oto isim:**  ${yazılacaksunucudb.isimler.giris ? yazılacaksunucudb.isimler.giris.replace(/<tag>/g, tagroldb.tag || "").replace(/<isim>/g, msg.author.username) : "Ayarlanmamış ❗"}`,
      ...
      async function otoisim(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 10, date: Date.now() }, "kur", "diğerleri")
      ...
      async function sembol(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 9, date: Date.now() }, "kur", "diğerleri")
      ...
      async function tag(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 8, date: Date.now() }, "kur", "diğerleri")
        let isim
      ...
      async function bot(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 7, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function erkek(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 6, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function kız(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 5, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function normal(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 50, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function seçenek(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 4, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function alınacak(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 3, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function yetkili(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 2, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function log(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 111, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function günlük(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: mesajId, idler: yazılacaksunucudb, f: 1, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function kayıtkanal(mesajId, funcMsg) {
        db.yaz(sunucuid, { channelId: msg.channelId, messageId: (mesajId || msg.id), idler: yazılacaksunucudb, f: 150, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
