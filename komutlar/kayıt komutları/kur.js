    ...
    try {
      ...
      let yazılacakguildDatabase = { isimler: {} }
        ...
        , m = {
          ...
          rolBul: (awaitMsg) => awaitMsg && (awaitMsg.mentions.roles.first() || awaitguild.roles.cache.get(awaitMsg.content.replace(/<@&|>/g, "").trim())),
          rolBulMulti: (awaitMsg) => {
            ...
            return awaitguild.roles.cache.filter(a => split.includes(a.id))
          },
          kanalBul: (awaitMsg) => awaitMsg && (awaitMsg.mentions.channels.first() || awaitguild.channels.cache.get(awaitMsg.content.replace(/<#|>/g, "").trim()))
        }
      if (sonradan) {
        const fun = sonradan.f
        yazılacakguildDatabase = sonradan.idler
        switch (fun) {
          case 150:
            return kayıtkanal(sonradan.mesajid, msg)
          case 1:
            return günlük(sonradan.mesajid, msg)
          case 111:
            return log(sonradan.mesajid, msg)
          case 2:
            return yetkili(sonradan.mesajid, msg)
          case 3:
            return alınacak(sonradan.mesajid, msg)
          case 4:
            return seçenek(sonradan.mesajid, msg)
          case 5:
            return kız(sonradan.mesajid, msg)
          case 50:
            return normal(sonradan.mesajid, msg)
          case 6:
            return erkek(sonradan.mesajid, msg)
          case 7:
            return bot(sonradan.mesajid, msg)
          case 8:
            return tag(sonradan.mesajid, msg)
          case 9:
            return sembol(sonradan.mesajid, msg)
          case 10:
            return otoisim(sonradan.mesajid, msg)
          default:
      ...
      async function son(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 11, date: Date.now() }, "kur", "diğerleri")
        ...
        let tagroldb = msg.client.tagrolDatabase(guildId)
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            },
            {
              ...
              value: [
                ...
                `**• Oto isim:**  ${yazılacakguildDatabase.isimler.giris ? yazılacakguildDatabase.isimler.giris.replace(/<tag>/g, tagroldb.tag || "").replace(/<isim>/g, msg.author.username) : "Ayarlanmamış ❗"}`,
          ...
          .setTimestamp()
        funcMsg.reply({ content: `• Kayıt sistemini test etmek için **${prefix}test** yazabilirsiniz!`, embeds: [embed] }).catch(() => { })
        db.yazdosya(guildDatabase, guildId)
        db.yaz(guildId, tagroldb, "tag rol", "diğerleri")
        db.sil(guildId, "kur", "diğerleri")
      }
      async function otoisim(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 10, date: Date.now() }, "kur", "diğerleri")
      ...
      async function sembol(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 9, date: Date.now() }, "kur", "diğerleri")
      ...
      async function tag(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 8, date: Date.now() }, "kur", "diğerleri")
        let isim
      ...
      async function bot(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 7, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function erkek(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 6, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function kız(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 5, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function normal(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 50, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function seçenek(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 4, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function alınacak(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 3, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function yetkili(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 2, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function log(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 111, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function günlük(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: mesajId, idler: yazılacakguildDatabase, f: 1, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
      ...
      async function kayıtkanal(mesajId, funcMsg) {
        db.yaz(guildId, { channelId: msg.channelId, messageId: (mesajId || msg.id), idler: yazılacakguildDatabase, f: 150, date: Date.now() }, "kur", "diğerleri")
        await channel?.awaitMessages({ filter: filter, max: 1, time: 45000 }).then(async a => {
          const mesaj = a.first()
          switch (mesaj.content.toLocaleLowerCase()) {
            case "kapat":
          ...
          let kanal = m.kanalBul(mesaj)
          if (kanal.type !== 0) return süre(kayıtkanal, m.kd, msg.id, mesaj)
          else if (kanal) {
            yazılacakguildDatabase.kanal = kanal.id
            mesajlar(m.g, mesaj)
            return await günlük(msg.id, mesaj)
          } else return süre(kayıtkanal, m.k, msg.id, mesaj)
        }).catch(err => süre(kayıtkanal, m.k), msg.id, (funcMsg || msg))
      }
      mesajlar(`${m.k}\n\n• İşlemi iptal etmek için **iptal** veya **kapat**\n• Eğer önceki soruya dönmek isterseniz **geri** yazabilirsiniz`, msg)
      kayıtkanal()
    } catch (e) {
