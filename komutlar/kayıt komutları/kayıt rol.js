...
module.exports = {
  ...
  async run({ guildDatabase, pre, alisa, msg, args, guildId, prefix, hata, guild, msgMember, guildMe }) {
    try {
      ...
      if (!guildDatabase.kayıt.secenek) return hata('Kayıt etme seçeneğiniz __**Cinsiyet**__ olarak ayarlı\n\n• Eğer normal olarak kaydetmek isterseniz **' + prefix + 'seç normal** yazabilirsiniz')
      if (args[0] === "sıfırla") {
        if (!guildDatabase.kayıt.normal) return hata('Üyelere verilecek olan rol zaten sıfırlanmış durumda')
      ...
      guildDatabase.kayıt.normal = rolmap
