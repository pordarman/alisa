        ...
        try {
            ...
            async function yaz() {
                ...
                await msg.channel?.awaitMessages({ filter: filter, max: 1, time: 1000 * 60 * 8 }).then(a => {
                    db.sil(guildId, "özel", "diğerleri")
                    let ms = a.first()
                    if (ms.content === 'iptal') return msg.reply({ content: "İşlem iptal edilmiştir" }).catch(() => { })
                    if (ms.content.toLocaleLowerCase() === 'sıfırla') {
                        if (!guildDatabase.kayıt.özel) return msg.reply({ content: "Özelleştirilmiş giriş mesajı zaten ayarlı değil" }).catch(() => { })
                        delete guildDatabase.kayıt.özel
                    ...
                    if (ms.content.length) {
                        ...
                        guildDatabase.kayıt.özel = { yazı: ms.content.replace(`<kutusuz>`, "").replace(image, ""), embed: embedlimi, im: image }
            ...
            if (db.bul(guildId, "özel", "diğerleri")) return hata(`Heyy dur bakalım orada! Şu anda başka bir yetkili özel mesajı ayarlıyor!`)
            db.yaz(guildId, { channelId: msg.channelId, messageId: msg.id, authorId: msg.author.id, date: Date.now() }, "özel", "diğerleri")
