...
module.exports = {
    ...
    async run({ int, guildDatabase, alisa, hata, guildId, guild }) {
        try {
            ...
            // Kontroller
            if (int.client.buttonCooldown.some(a => a == int.user.id)) return hata(`Heyyy dur bakalım orada! Zaten halihazırda bir kayıt işlemi gerçekleştiriyorsun!`)
            ...
            await member.edit({ roles: [...botrolid, ...member.roles.cache.filter(a => a.id != kayıtsızrolid).map(a => a.id)], nick: ismi }).then(async () => {
                ...
                let kontrolisimler = guildDatabase.kayıt[memberid]
