...
module.exports = {
    ...
    async run({ int, guildDatabase, alisa, hata, sonradan, guildId, guild }) {
        try {
                ...
                return await int.channel.awaitMessages({ filter: filter, time: 30000, max: 1 }).then(async mesaj1 => {
                    ...
                    return await member.edit({ roles: [...verilecekRolId, ...member.roles.cache.filter(a => ![kayıtsızrolid, ...verilecekRolId].includes(a.id)).map(a => a.id)], nick: ismi }).then(async () => {
                        ...
                        let toplamherkes = db.topla(guildId, 1, "kayıt toplam herkes", "diğerleri")
                            ...
                            , isimler = { c: ayarlar.emoji.kiz, n: ismi, r: verilecekRolString, s: sahipid, z: date2 }
            ...
            let erkekrolseysi = guildDatabase.kayıt.erkek || []
            if ([...verilecekRolId, ...erkekrolseysi].some(a => member.roles.cache.has(a))) return hata('Kayıt etmek istediğiniz kişi zaten daha önceden kayıt edilmiş')
            let rolVarMı = true
            ...
            int.message.reply({ content: `${ayarlar.emoji.kiz} <@${sahipid}>, kayıt etmek istediğiniz <@${memberid}> adlı kişinin sadece ismini ${guildDatabase.kayıt.yaszorunlu ? "ve yaşını" : ""} mesaj olarak yazınız` }).then(async ms => {
