    ...
    async run({ int, sunucudb, alisa, hata, sunucuid, guild }) {
        try {
            ...
            let dugme = new ActionRowBuilder()
                , sayı = +embeds.text.match(/\d+(?=\/)/)[0] + 5
                , sayfa = embeds.text.match(/(?<=\/)\d+/)[0]
            if (sayı > sayfa) sayı = sayfa
            int.message.components[0].components.forEach(a => {
                if (["yardım_komutlar_sağok", "yardım_komutlar_saghizli"].some(aa => a.data.custom_id.startsWith(aa)) && sayı == sayfa) a.data.disabled = true
