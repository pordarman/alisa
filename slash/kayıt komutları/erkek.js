...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName("erkek")
        .setDescription("Üyeyi erkek olarak kayıt et")
    ...
    async run({ int, sunucudb, alisa, hata, sunucuid, guild }) {
        try {
            ...
            if (member.user.bot) {
                if (sunucudb.kayıt.bot) return hata(`Bir botu erkek olarak kayıt etemezsin şapşik şey seni\n\n• Eğer botu kayıt etmek isterseniz **${prefix}bot ${member.id}** yazabilirsiniz`)
                if (intMember.permissions.has('Administrator')) return hata('Bir botu erkek olarak kayıt etemezsin şapşik şey seni\n\n• Eğer botu kayıt etmek isterseniz ilk önce **' + prefix + 'bot-rol** ile bir bot rolünü ayarlamalısınız')
            ...
            let kızrolseysi = sunucudb.kayıt.kız || [], rolVarMı = true
