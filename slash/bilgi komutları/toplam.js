...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName("toplam")
        .setDescription("Sunucunun kayıt ettiği toplam kişi sayısını gösterir"),
    /**
        ...
        try {
            ...
            if (ssayı.length) {
                ...
                if (guildDatabase.kayıt.secenek) {
                    ...
                    fields.push({ name: 'KAYIT EDİLEN (' + (ssayı.length.toLocaleString().replace(/\./g, ",")) + ')', value: `${ayarlar.emoji.uye} **Üye:**  ${normaltoplam.toLocaleString().replace(/\./g, ",")}\n🤖 **Bot:**  ${(ssayı.length - erkektoplam - kıztoplam - normaltoplam).toLocaleString().replace(/\./g, ",")}`, inline: true })
                } else {
                    embed.setDescription('**Kayıt seçeneğiniz:**  👫 Cinsiyet')
                    fields.push({ name: 'KAYIT EDİLEN (' + (ssayı.length.toLocaleString().replace(/\./g, ",")) + ')', value: `${ayarlar.emoji.erkek} **Erkek:**  ${erkektoplam.toLocaleString().replace(/\./g, ",")}\n${ayarlar.emoji.kiz} **Kız:**  ${kıztoplam}\n🤖 **Bot:**  ${(ssayı.length - erkektoplam - kıztoplam - normaltoplam).toLocaleString().replace(/\./g, ",")}`, inline: true })
                }
                let ranklar = ayarlar.ranklar
                const uuu = Object.entries(guildDatabase.kayıtkisiler).filter(a => a[1].toplam != undefined).sort((a, b) => b[1].toplam - a[1].toplam).slice(0, 3).map((a, i) => {
                    if (a[0] == int.user.id) return `• ${int.client.stringToEmojis(i + 1)} **<@${a[0]}> ${a[1].toplam || "0"} Kayıt sayın • ${ranklar[a[1].rank] || "Rankın yok"}**`
                ...
                if (guildDatabase.kayıt.secenek) {
                    embed.setDescription('**Kayıt seçeneğiniz:**  👤 Normal Kayıt')
                    fields.push({ name: 'KAYIT EDİLEN (0)', value: `${ayarlar.emoji.uye} **Üye:**  0\n🤖 **Bot:**  0`, inline: true })
                } else {
                    embed.setDescription('**Kayıt seçeneğiniz:**  👫 Cinsiyet')
                    fields.push({ name: 'KAYIT EDİLEN (0)', value: `${ayarlar.emoji.erkek} **Erkek:**  0\n${ayarlar.emoji.kiz} **Kız:**  ${kıztoplam}\n🤖 **Bot:**  ${(ssayı.length - erkektoplam - kıztoplam - normaltoplam).toLocaleString().replace(/\./g, ",")}`, inline: true })
                }
