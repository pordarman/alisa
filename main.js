...
const { Collection, AttachmentBuilder, Client, GatewayIntentBits, User } = Discord

const ayarlar = require('./ayarlar.json')

const client = new Client({ restRequestTimeout: 60000, intents: [GatewayIntentBits.Guilds, GatewayIntentBits.MessageContent, GatewayIntentBits.GuildMessages, GatewayIntentBits.GuildMembers, GatewayIntentBits.GuildVoiceStates, GatewayIntentBits.GuildIntegrations, GatewayIntentBits.GuildBans], presence: { status: "idle" }, failIfNotExists: false, shardCount: ayarlar.shard })
...
/* 
  * Slash komutlarını JSON olarak burada tutuyoruz
  */
...
/* 
  ...
  */
client.s = (id) => client.sunucudb[id] || db.buldosya(id)


/* 
  * Sunucunun tagrol veri bilgisini çeker
  */
client.t = (id, tag) => db.bul(id, "tag rol", "diğerleri") || { kisi: {}, tag: (tag ? tag.slice(0, -1) : undefined) }


/* 
  * ID'sini girdiğiniz sunucunun shard ID'sini yazar
  */
client.shardId = (guildId) => Number(BigInt(guildId) >> 22n) % ayarlar.shard
...
/* 
  ...
  */
client.namePhoto = {
  üye: new AttachmentBuilder("https://i.hizliresim.com/emr7apu.png", { name: "örnek_kullanım.png" }),
  şüpheli: new AttachmentBuilder("https://i.hizliresim.com/befowjp.png", { name: "örnek_kullanım.png" }),
  yeniden: new AttachmentBuilder("https://i.hizliresim.com/6vnmf5k.png", { name: "örnek_kullanım.png" }),
  erkek: new AttachmentBuilder("https://i.hizliresim.com/isk8bbl.png", { name: "örnek_kullanım.png" }),
  kız: new AttachmentBuilder("https://i.hizliresim.com/6w8153u.png", { name: "örnek_kullanım.png" }),
  bot: new AttachmentBuilder("https://i.hizliresim.com/nmntmn8.png", { name: "örnek_kullanım.png" }),
  isim: new AttachmentBuilder("https://i.hizliresim.com/s190sc5.png", { name: "örnek_kullanım.png" })
}
...
/* 
  ...
  */
client.hata = (dirname, error) => client.hook?.send(`**${dirname}** adlı komut dosyamda bir hata oluştu!\n` + "```js\n" + error + "```")
...
client.fetchUserForce = async (id) => {
  ...
  if (user) return user
  try {
    let deneme = await client.shard.broadcastEval((c, idg) => {
...
client.getGuildNameOrId = async (id, bool = true) => {
  try {
    ...
    sunucu = await client.shard.broadcastEval((c, id) => c.guilds.cache.get(id), { context: id, shard: client.shardId(id) })
    if (sunucu) return bool ? `**${sunucu.name}**` : sunucu.name
  } catch (e) { }
...
client.sendChannel = async (object, id, guildId) => {
  try {
    ...
    return await client.shard.broadcastEval((a, array) => {
      let kanalClient = a.channels.cache.get(array[1])
      if (kanalClient) return kanalClient.send(array[0]).catch(err => { })
    }, { context: [object, id], shard: (guildId ? client.sharId(guildId) : undefined) })
  } catch (e) {
    return undefined
  }
}
...
client.getGuild = async (id) => {
  try {
    ...
    return await client.shard.broadcastEval((a, id_1) => {
...
readdirSync(`${__dirname}\\butonlar`).forEach(klasorAdları => {
  readdirSync(`${__dirname}\\butonlar\\${klasorAdları}`).forEach(file => {
    ...
    client.butonlar.set(command.name, command)
  })
});
...
readdirSync(`${__dirname}\\komutlar`).forEach(klasorAdları => {
  readdirSync(`${__dirname}\\komutlar\\${klasorAdları}`).forEach(file => {
    ...
    command.kod.forEach(x => client.commands.set(x, command))
  })
});
